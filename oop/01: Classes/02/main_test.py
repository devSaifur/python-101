...
def test(wall, expected_output):
    ...
    try:
        print(
            f"Expected Wall - volume: {wall.volume} depth: {wall.depth} height: {wall.height} width: {wall.width}"
        )
        print(
            f"Actual Wall   - volume: {expected_volume} depth: {expected_depth} height: {expected_height} width: {expected_width}"
        )
        if (
